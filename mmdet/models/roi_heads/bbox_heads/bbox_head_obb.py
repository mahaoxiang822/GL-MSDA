...
from mmdet.core import (multi_apply, xywhtheta_to_rect_grasp_group, xywhtheta_to_points_graspnet,
...
class BBoxHeadOBB(nn.Module):
    ...
    def __init__(self,
        ...
        self.score_target_mean = score_target_mean
        self.score_target_std = score_target_std
        ...
        out_dim_reg_grasp = 5 if reg_class_agnostic else 5 * num_classes
    ...
    def _get_target_single(self,
        ...
        pos_rect_grasps_xywhtheta = points_to_xywhtheta_graspnet(pos_bboxes)

        if num_pos > 0:
            labels[:num_pos] = 0
            pos_gt_scores = (pos_gt_scores.view(-1, 1)[:num_pos] - self.score_target_mean) / self.score_target_std
            score_targets[:num_pos] = pos_gt_scores
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight
            label_weights[:num_pos] = pos_weight
            score_weights[:num_pos, :] = 1
            pos_bbox_targets = xywhtheta_to_delta(pos_rect_grasps_xywhtheta,
                                                  pos_gt_rect_grasps,
                                                  self.target_means,
                                                  self.target_stds)
    ...
    def get_det_rect_grasp_group(self,
        ...
        if rrois.size(1) == 6:
            ...
            assert rrois.size(1) != 6

        # hbboxes = hbboxes[mask]
        # scores = scores[mask]
        # bbox_pred = bbox_pred[mask]
        # score_pred = score_pred[mask]
        rbboxes = delta_to_xywhtheta(bbox_pred, rbboxes, self.target_means,
                                           self.target_stds)
        ...
        if dataset == 'graspnet':
            ...
            rect_grasps = xywhtheta_to_rect_grasp_group(rbboxes, score_pred, ori_shape)
            return rect_grasps
        else:
            rbboxes_poly = xywhtheta_to_points_graspnet(rbboxes, ori_shape)
