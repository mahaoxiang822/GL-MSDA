...
class BBoxHeadGraspNetDepthDA(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_grasp=dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0)                 ):
        super(BBoxHeadGraspNetDepthDA, self).__init__()
    ...
    def _get_target_single(self,
        ...
        if domain == 'target':
            domain_masks = labels.new_zeros((num_samples,), dtype=torch.long)
        else:
            domain_masks = labels.new_ones((num_samples,), dtype=torch.long)
        ...
        return labels, label_weights, score_targets, score_weights, bbox_targets, bbox_weights, domain_masks
    ...
    def get_targets(self,
        ...
        bbox_targets, bbox_weights, domain_masks = multi_apply(
        ...
        if concat:
            ...
            domain_masks = torch.cat(domain_masks, 0)
        return labels, label_weights, score_targets, score_weights, bbox_targets, bbox_weights, domain_masks
    ...
    def loss(self,
        ...
        losses = dict()
        domain_masks = domain_masks.type(torch.bool)
        source_num = len(domain_masks[domain_masks])

        # cls_score = cls_score[domain_masks]
        # score_pred = score_pred[domain_masks]
        # bbox_pred = bbox_pred[domain_masks]
        # labels = labels[domain_masks]
        # label_weights = label_weights[domain_masks]
        # score_targets = score_targets[domain_masks]
        # score_weights = score_weights[domain_masks]
        # bbox_targets = bbox_targets[domain_masks]
        # bbox_weights = bbox_weights[domain_masks]


        if cls_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            label_weights[~domain_masks] = label_weights[~domain_masks] * 0.0
        ...
        if bbox_pred is not None:
            score_weights[~domain_masks] = score_weights[~domain_masks] * 0.0
            bbox_weights[~domain_masks] = bbox_weights[~domain_masks] * 0.0
        ...
        #     losses['loss_score'] = losses['loss_score'] * 0.0
        #     losses['loss_grasp'] = losses['loss_grasp'] * 0.0
