...
class StandardRoIHeadBuildGP(BaseRoIHeadOBB):
    ...
    def init_fa(self, calGP=False, fa_classes=12, gp_path=None):
        ...
        self.GPS = torch.zeros(2048, fa_classes, device='cpu')
        self.GPT = torch.zeros(2048, fa_classes, device='cpu')
    ...
    def forward_train(self,
        ...
        if not self.loaded:
            # self.GPS = torch.load(self.gp_path + 'no_fc_gps_tmp.pth', map_location='cpu')
            # self.GPT = torch.load(self.gp_path + 'no_fc_gpt_tmp.pth', map_location='cpu')
            print('************************************************')
        ...
        if self.with_bbox:
            ...
            for myi in range(self.fa_classes):
                if nums[myi] > 0:
                    self.GPT[:, myi] = (self.GPT[:, myi] * self.target_nums[myi] + PT[:, myi]) / (
                                self.target_nums[myi] + nums[myi])
                    self.target_nums[myi] = self.target_nums[myi] + nums[myi]
            ...
            for myi in range(self.fa_classes):
                if nums[myi] > 0:
                    self.GPS[:, myi] = (self.GPS[:, myi] * self.source_nums[myi] + PS[:, myi]) / (
                                self.source_nums[myi] + nums[myi])
                    self.source_nums[myi] = self.source_nums[myi] + nums[myi]
                    PS[:, myi] = PS[:, myi] / nums[myi]
            print('PS:',torch.norm(PS[:,0]),'GPS:',torch.norm(self.GPS[:,0]))
            print('PT:',torch.norm(PT[:,0]),'GPT:',torch.norm(self.GPT[:,0]))
            self.save_gps(self.gp_path + 'gps_tmp.pth')
            self.save_gpt(self.gp_path + 'gpt_tmp.pth')
    ...
    def save_gps(self, gp_path):
        torch.save(self.GPS, gp_path)

    def save_gpt(self, gp_path):
        torch.save(self.GPT, gp_path)
