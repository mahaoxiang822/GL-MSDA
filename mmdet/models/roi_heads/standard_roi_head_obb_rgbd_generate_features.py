...
class StandardRoIHeadGenerateFeatures(BaseRoIHeadOBB):
    ...
    def init_fa(self, calGP=False, fa_classes=12, gp_path=None):
        ...
        self.TSNE_feature_nums = 5000
        self.TSNE_features = torch.zeros(2, fa_classes, self.TSNE_feature_nums, 2048)
        self.TSNE_nums = torch.zeros(2, fa_classes)
    ...
    def forward_train(self,
        ...
        if not self.loaded:
            self.loaded = True
            device = x[0].device
            self.TSNE_features = self.TSNE_features.to(device)
        ...
        if self.with_bbox:
            ...
            for myi in range(len(target_index)):
                ...
                if ind == 12:
                    ind -= 1
                if self.TSNE_nums[0, ind] < self.TSNE_feature_nums:
                    p = random.uniform(0, 1)
                    if p < 0.1:
                        self.TSNE_features[0, ind, self.TSNE_nums[0, ind], :] = target_features[myi]
                        self.TSNE_nums[0, ind] = self.TSNE_nums[0, ind] + 1
            ...
            for myi in range(len(source_index)):
                ...
                if ind == 12:
                    ind -= 1
                if self.TSNE_nums[1, ind] < self.TSNE_feature_nums:
                    p = random.uniform(0, 1)
                    if p < 0.1:
                        self.TSNE_features[1, ind, self.TSNE_nums[1, ind], :] = source_features[myi]
                        self.TSNE_nums[1, ind] = self.TSNE_nums[1, ind] + 1

            if torch.min(self.TSNE_nums).item() > self.TSNE_feature_nums - 1:
                torch.save(self.TSNE_features, self.gp_path + 'TSNE_features.pth')
                exit()

            print(self.TSNE_nums)
            print(torch.min(self.TSNE_nums))
