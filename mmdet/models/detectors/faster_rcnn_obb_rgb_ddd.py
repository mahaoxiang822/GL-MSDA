...
class FasterRCNNOBBRGBDDD(BaseDetector):
    ...
    def __init__(self,
        ...
        super(FasterRCNNOBBRGBDDD, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(FasterRCNNOBBRGBDDD, self).init_weights(pretrained)
    ...
    def extract_feat_cross_d2rgb_k(self, rgb, depth, img_metas=None):
        ...
        for i, layer_name in enumerate(self.rgb_backbone.res_layers):
            ...
            if i == 0:
                x_rgb = self.fusion[i](query=x_rgb, key=x_depth[i], img_metas=img_metas)
            else:
                x_rgb = self.fusion[i](query=x_rgb, key=x_depth[i])
    ...
    def extract_feat_concat(self, rgb, depth):
        depth = depth.repeat(1, 3, 1, 1)
        x_depth = self.depth_backbone(depth)
    ...
    def forward_train(self,
        ...
        if self.fusion_type == 'concat':
            x = self.extract_feat_concat(rgb, depth)
